...
Both Experiment 1 and Experiment 2 use the English ANEW [@Bradley1999] norms to create stimuli for this study, in an effort to replicate @Jasmin2012 experiments, and `r length(table(exp1data$trialcode))` words were selected for this experiment. Pseudowords were selected from Appendix E of the supplementary materials presented from the QWERTY publication. These words were coded as described below for RSA, switches, word length, and letter frequency. Average word length was `r apa(mean(exp1data$length), 2)` (*SD* = `r apa(sd(exp1data$length), 2)`; range = `r min(exp1data$length)` - `r max(exp1data$length)`). All materials, data, and the *R*markdown document that created this manuscript are avaliable at our Open Science Foundation (OSF) page: https://osf.io/zs2qj/. 
...
After setting  participants as a random intercept factor, letter frequency was used as an adjustor variable. As seen in Table \@ref(tab:model-table1), this variable was not a significant predictor for pseudowords, *b* = `r apa(summary(model3f)$tTable[2,1],2)`, but was a significant predictor for real words, *b* = `r apa(summary(model3r)$tTable[2,1],2)`. All predictor statistics are provided in an Excel sheet on the OSF page for each step of the model. Next, the main effects of typing speed, hand switches, finger switches, and RSA were added to the models for pseudowords and real words. In both models, the addition of these variables overall was significant, *p* < .001. For psuedowords, typing speed was not a significant predictor of valence ratings, *b* = `r apa(summary(model4f)$tTable[3,1], 2)`, *t*(`r summary(model4f)$tTable[3,3]`) = `r apa(summary(model4f)$tTable[3,4],2)`, *p* = `r apa(summary(model4f)$tTable[3,5],3,F)`. Similarly, typing speed was not a significant predictor for valence ratings on real words, *b* = `r apa(summary(model4r)$tTable[3,1], 2)`, *t*(`r summary(model4r)$tTable[3,3]`) = `r apa(summary(model4r)$tTable[3,4],2)`, *p* = `r apa(summary(model4r)$tTable[3,5],3,F)`. In contrast, the measures of typability in hand and finger switching were significant for both pseudowords and real words. For pseudowords, increased hand switching, *b* = `r apa(summary(model4f)$tTable[4,1], 2)`, *t*(`r summary(model4f)$tTable[4,3]`) = `r apa(summary(model4f)$tTable[4,4],2)`, *p* = `r apa(summary(model4f)$tTable[4,5],3,F)`, and increased finger switching, *b* = `r apa(summary(model4f)$tTable[5,1], 2)`, *t*(`r summary(model4f)$tTable[5,3]`) = `r apa(summary(model4f)$tTable[5,4],2)`, *p* < .001, decreased the overall valence ratings. However increased hand switching, *b* = `r apa(summary(model4r)$tTable[4,1], 2)`, *t*(`r summary(model4r)$tTable[4,3]`) = `r apa(summary(model4r)$tTable[4,4],2)`, *p* = `r apa(summary(model4r)$tTable[4,5],3,F)`, increased valence ratings for real words, while increased finger switching, *b* = `r apa(summary(model4r)$tTable[5,1], 2)`, *t*(`r summary(model4r)$tTable[5,3]`) = `r apa(summary(model4r)$tTable[5,4],2)`, *p* < .001, decreased the overall valence ratings. Even adjusting for these typing style variables, the RSA effect replicated for both pseudowords, *b* = `r apa(summary(model4f)$tTable[6,1], 2)`, *t*(`r summary(model4f)$tTable[6,3]`) = `r apa(summary(model4f)$tTable[6,4],2)`, *p* < .001, and real words, *b* = `r apa(summary(model4r)$tTable[6,1], 2)`, *t*(`r summary(model4r)$tTable[6,3]`) = `r apa(summary(model4r)$tTable[6,4],2)`, *p* < .001. In the next section, we explored the interactions of typability and RSA, to present a more nuanced view of typing's effect on valence ratings.
...
write.csv(excelprint, "Predictor Statistics Exp 1.csv", row.names = F)
...
write.csv(excelprint, "Interaction Statistics Exp 1.csv", row.names = F)
...
For pseudowords, finger switches by RSA, *b* = `r apa(summary(model5f)$tTable[12,1], 2)`, *t*(`r summary(model5f)$tTable[12,3]`) = `r apa(summary(model5f)$tTable[12,4],2)`, *p* < .001, and typing speed by RSA, *b* = `r apa(summary(model5f)$tTable[10,1], 2)`, *t*(`r summary(model5f)$tTable[10,3]`) = `r apa(summary(model5f)$tTable[10,4],2)`, *p* = `r apa(summary(model5f)$tTable[10,5],3,F)` were the only significant interactions. Low and high simpel slopes for RSA were created to examine the effects of typing speed and finger switches at these levels. For low RSA (more left handed words), speed positively predicted valence, *b* = `r apa(summary(model5fl)$tTable[3,1], 2)`, *t*(`r summary(model5fl)$tTable[3,3]`) = `r apa(summary(model5fl)$tTable[3,4],2)`, *p* = `r apa(summary(model5fl)$tTable[3,5],3,F)`, and finger switching negatively predicted valence, *b* = `r apa(summary(model5fl)$tTable[5,1], 2)`, *t*(`r summary(model5fl)$tTable[5,3]`) = `r apa(summary(model5fl)$tTable[5,4],2)`, *p* < .001. For average RSA, speed no longer predicted valence, *b* = `r apa(summary(model5f)$tTable[3,1], 2)`, *t*(`r summary(model5f)$tTable[3,3]`) = `r apa(summary(model5f)$tTable[3,4],2)`, *p* = `r apa(summary(model5f)$tTable[3,5],3,F)`, while finger switches still negatively predicted valence, albeit smaller than at low RSA, *b* = `r apa(summary(model5f)$tTable[5,1], 2)`, *t*(`r summary(model5f)$tTable[5,3]`) = `r apa(summary(model5f)$tTable[5,4],2)`, *p* = `r apa(summary(model5f)$tTable[5,5],3,F)`. Last, at high RSA (more right handed words), speed did not predict valence, *b* = `r apa(summary(model5fh)$tTable[3,1], 2)`, *t*(`r summary(model5fh)$tTable[3,3]`) = `r apa(summary(model5fh)$tTable[3,4],2)`, *p* = `r apa(summary(model5fh)$tTable[3,5],3,F)`, and neither did finger switches, *b* = `r apa(summary(model5fh)$tTable[5,1], 2)`, *t*(`r summary(model5fh)$tTable[5,3]`) = `r apa(summary(model5fh)$tTable[5,4],2)`, *p* = `r apa(summary(model5fh)$tTable[5,5],3,F)`. In sum, this interaction indicates that typing speed was a positive influence for more left handed words, but was not a predictor as words were typed more with the right hand. When words were more left handed, there was a negative influence of finger switching, but as we transition to more right handed words the number of switches did not influence valence ratings. These results seem to indicate that expertise and typability were influential factors for left handed words, but the RSA washed out these effects when rating right handed psuedowords. 

### Real Word Simple Slopes
For real words, the three-way interactions of finger switch by hand switch by RSA, *b* = `r apa(summary(model5r)$tTable[16,1], 2)`, *t*(`r summary(model5r)$tTable[16,3]`) = `r apa(summary(model5r)$tTable[16,4],2)`, *p* < .001, and speed by finger switch by hand switch, *b* = `r apa(summary(model5r)$tTable[13,1], 2)`, *t*(`r summary(model5r)$tTable[13,3]`) = `r apa(summary(model5r)$tTable[13,4],2)`, *p* = `r apa(summary(model5r)$tTable[13,5],3,F)`, were the largest significant interaction predictors. Low and high simple slopes for finger switches were created to explore the three-way interaction. For lower finger switches, the hand switching by RSA interaction was significant, *b* = `r apa(summary(model5rl)$tTable[11,1], 2)`, *t*(`r summary(model5rl)$tTable[11,3]`) = `r apa(summary(model5rl)$tTable[11,4],2)`, *p* = `r apa(summary(model5rl)$tTable[11,5],3,F)`; however, the hand switches by speed interaction was not significant, *b* = `r apa(summary(model5rl)$tTable[7,1], 2)`, *t*(`r summary(model5rl)$tTable[7,3]`) = `r apa(summary(model5rl)$tTable[7,4],2)`, *p* = `r apa(summary(model5rl)$tTable[7,5],3,F)`. At average finger switching, the hand switches by RSA interaction was not significant, *b* = `r apa(summary(model5r)$tTable[11,1], 2)`, *t*(`r summary(model5r)$tTable[11,3]`) = `r apa(summary(model5r)$tTable[11,4],2)`, *p* = `r apa(summary(model5r)$tTable[11,5],3,F)`, and neither was the hand switches by speed interaction, *b* = `r apa(summary(model5r)$tTable[7,1], 2)`, *t*(`r summary(model5r)$tTable[7,3]`) = `r apa(summary(model5r)$tTable[7,4],2)`, *p* = `r apa(summary(model5r)$tTable[7,5],3,F)`. At a higher number of finger switches the hand switches by RSA interaction was significant, *b* = `r apa(summary(model5rh)$tTable[11,1], 2)`, *t*(`r summary(model5rh)$tTable[11,3]`) = `r apa(summary(model5rh)$tTable[11,4],2)`, *p* = `r apa(summary(model5rh)$tTable[11,5],3,F)`, along with the hand switches by speed interaction, *b* = `r apa(summary(model5rh)$tTable[13,1], 2)`, *t*(`r summary(model5rh)$tTable[13,3]`) = `r apa(summary(model5rh)$tTable[13,4],2)`, *p* = `r apa(summary(model5rh)$tTable[13,5],3,F)`.

For significant two-way effects of hand switch by RSA or hand switch by speed, we then calculated the low and high simple slopes for hand switches. Therefore, we explored the low and high finger switch effects that were significant with low and high hand switches for RSA and speed main effects. At low finger switches and low hand switches, RSA was a significant predictor of valence, *b* = `r apa(summary(model5rll)$tTable[6,1], 2)`, *t*(`r summary(model5rll)$tTable[6,3]`) = `r apa(summary(model5rll)$tTable[6,4],2)`, *p* = `r apa(summary(model5rll)$tTable[6,5],3,F)`. Speed was not examined because the two-way interaction was not significant. At low finger switches and average hand switches, RSA was a stronger predictor of valence, *b* = `r apa(summary(model5rl)$tTable[6,1], 2)`, *t*(`r summary(model5rl)$tTable[6,3]`) = `r apa(summary(model5rl)$tTable[6,4],2)`, *p* < .001. Last, at low finger switches and high hand switches, RSA increased in strength, *b* = `r apa(summary(model5rlh)$tTable[6,1], 2)`, *t*(`r summary(model5rlh)$tTable[6,3]`) = `r apa(summary(model5rlh)$tTable[6,4],2)`, *p* = `r apa(summary(model5rlh)$tTable[6,5],3,F)`. Therefore, at low numbers of finger switches, as hand switching increased, the strength of the RSA positivity effect also increased. This result implies that as words were switching hands, the more right handed letters a word has during these switches increased valence. 

At a high number of finger switches, we found both speed and RSA interactions with hand switching. When there were low numbers of hand switches for these words, RSA was a positive significant predictor, *b* = `r apa(summary(model5rhl)$tTable[6,1], 2)`, *t*(`r summary(model5rhl)$tTable[6,3]`) = `r apa(summary(model5rhl)$tTable[6,4],2)`, *p* < .001, along with speed, *b* = `r apa(summary(model5rhl)$tTable[3,1], 2)`, *t*(`r summary(model5rhl)$tTable[3,3]`) = `r apa(summary(model5rhl)$tTable[3,4],2)`, *p* = `r apa(summary(model5rhl)$tTable[3,5],3,F)`. As hand switches increase, the effects of RSA and speed decrease. For high finger switches and average hand switches, RSA was significant, *b* = `r apa(summary(model5rh)$tTable[6,1], 2)`, *t*(`r summary(model5rh)$tTable[6,3]`) = `r apa(summary(model5rh)$tTable[6,4],2)`, *p* < .001`, while speed was not *b* = `r apa(summary(model5rh)$tTable[3,1], 2)`, *t*(`r summary(model5rh)$tTable[3,3]`) = `r apa(summary(model5rh)$tTable[3,4],2)`, *p* = `r apa(summary(model5rh)$tTable[3,5],3,F)`. With high finger and hand switches, RSA was significant but smaller than low and average, *b* = `r apa(summary(model5rhh)$tTable[6,1], 2)`, *t*(`r summary(model5rhh)$tTable[6,3]`) = `r apa(summary(model5rhh)$tTable[6,4],2)`, *p* < .001`, and speed was not a significant predictor, *b* = `r apa(summary(model5rhh)$tTable[3,1], 2)`, *t*(`r summary(model5rhh)$tTable[3,3]`) = `r apa(summary(model5rhh)$tTable[3,4],2)`, *p* = `r apa(summary(model5rhh)$tTable[3,5],3,F)`. Therefore, at an elevated number of finger switches, we found that RSA and speed were positive predictors at low hand switches. As hand switching and finger switching increases, the effects of expertise and RSA decreased. This result implies that the coordination of controlling for finger and hand switching decreased the positive valence effects of RSA and expertise. All interaction statistics are included online in an Excel sheet at our OSF page. 
...
FOUR WAY INTERACTION FOR PSUEDO WORDS UGH WHY
WHAT CAN WE DO HERE WHAT THE FUCK

Break down by speed 
summary(model5fl) low speed
-lrswitch:fingerswitch:rha 16 is not sig

summary(model5f) average speed
-lrswitch:fingerswitch:rha 16 is sig

summary(model5fh) high speed
-lrswitch:fingerswitch:rha 16 is sig

Break down by hand switches
--so we've done hand switches and speed, so theres only finger switches and RSA left

#summary(model5fal) average speed low hand switch
-fingerswitch:rha 12 is sig

#summary(model5f) average speed average hand switch
-fingerswitch:rha 12 is sig

#summary(model5fah) average speed high hand switch 
-summary(model5fah)

summary(model5fhl) high speed low hand switch
-fingerswitch:rha 12 is NOT

summary(model5fh) high speed average hand switch
-fingerswitch:rha 12 is sig

summary(model5fhh) high speed high hand switch 
-fingerswitch:rha 12 is sig

Break down by finger switch for the significant interactions

summary(model5fall) average speed, low hand, low fingerf
-rsa 6 .08 is sig

summary(model5fala) average speed, low hand, average finger
-rsa 6 .06 is sig

summary(model5falh) average speed, low hand, high finger
-rsa 6 .04 is sig

summary(model5faal) average speed, average hand, low finger
summary(model5f) average speed, average hand, average finger
summary(model5faah) average speed, average hand, high finger 

summary(model5fahl) average speed, high hand, low finger
summary(model5faha) average speed, high hand, average finger
summary(model5fahh) average speed, high hand, high finger

summary(model5fhal) high speed, average hand, low finger
summary(model5fhaa) high speed, average hand, average finger
summary(model5fhah) high speed, average hand, high finger 

summary(model5fhhl) high speed, high hand, low finger
summary(model5fhha) high speed, high hand, average finger
summary(model5fhhh) high speed, high hand, high finger 

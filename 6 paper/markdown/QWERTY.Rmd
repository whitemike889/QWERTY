...
summary(model5fl)$tTable
summary(model5f)$tTable
summary(model5fh)$tTable
summary(model5rl)$tTable
summary(model5r)$tTable
summary(model5rh)$tTable
summary(model5rll)$tTable
summary(model5rlh)$tTable
summary(model5rhl)$tTable
summary(model5rhh)$tTable
...
For pseudowords, finger switches by RSA, *b* = `r apa(summary(model5f)$tTable[12,1], 2)`, *t*(`r summary(model5f)$tTable[12,3]`) = `r apa(summary(model5f)$tTable[12,4],2)`, *p* < .001, and typing speed by RSA, *b* = `r apa(summary(model5f)$tTable[10,1], 2)`, *t*(`r summary(model5f)$tTable[10,3]`) = `r apa(summary(model5f)$tTable[10,4],2)`, *p* = `r apa(summary(model5f)$tTable[10,5],3,F)` were the only significant interactions. 

HERE I THINK WE SHOULD BREAK THIS DOWN BY RSA SINCE IT'S THE SIMILAR ONE BETWEEN THE TWO. 

##increasing RSA leads to:
##less importance of expertise (speed variable goes from positive to closer to zero)
##less importance of fingerswitching (goes from negative to closer to zero)

--  so to explain why finger switch is negative we may be able to relate this to captcha ... in thinking about how we type those, we usually type them with one finger so switching is not so big deal 
-- so as it's more left, we hate that it switches fingers, which leads to the RSA ... as it's more right handed the number of switches doesn't matter
-- as it's more left, typing speed is important and plays a positive role, as it's more right, speed not so important. 


### Real Word Simple Slopes
For real words, the three-way interactions of finger switch by hand switch by RSA, *b* = `r apa(summary(model5r)$tTable[16,1], 2)`, *t*(`r summary(model5r)$tTable[16,3]`) = `r apa(summary(model5r)$tTable[16,4],2)`, *p* < .001, and speed by finger switch by hand switch, *b* = `r apa(summary(model5r)$tTable[13,1], 2)`, *t*(`r summary(model5r)$tTable[13,3]`) = `r apa(summary(model5r)$tTable[13,4],2)`, *p* = `r apa(summary(model5r)$tTable[13,5],3,F)`, were the largest significant interaction predictors.


BREAK THIS DOWN BY FINGER, THEN HAND, SEE WHAT HAPPENS WITH SPEED AND RSA

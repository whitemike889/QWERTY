...
excelprint[3, 1:3] = c("Psuedo", "Control Variables",  rownames(summary(model3f)$tTable)[1])
excelprint[4, 1:3] = c("Psuedo", "Control Variables",  rownames(summary(model3f)$tTable)[2])
...
excelprint[30, 1:3] = c("Real", "Control Variables",  rownames(summary(model3r)$tTable)[1])
excelprint[31, 1:3] = c("Real", "Control Variables",  rownames(summary(model3r)$tTable)[2])
...
Next, the four-way interaction of typing speed, finger switching, hand switching, and RSA was entered into the equation, including all the smaller two- and three-way interactions. We focused on the largest interaction found, breaking down interaction terms into simple slopes of low (-1SD), average, and high (+1SD) to explore each effect. For example, if the four-way interaction was significant, one variable would be broken into simple slopes, and the next largest interactions would be examined. This procedure was iterated until the interactions were no longer significant or the effects were down to single variables. When multiple interactions were present, we choose a common variable to help break down the interactions with the least number of steps. Table \@ref(tab:model-table1) portrays that the pseudoword, *p* = .003, and real word, *p* < .001, models were significantly different than the main effects only models. 

For pseudowords, finger switches X RSA, *b* = `r apa(summary(model5f)$tTable[12,1], 2)`, *t*(`r summary(model5f)$tTable[12,3]`) = `r apa(summary(model5f)$tTable[12,4],2)`, *p* < .001, and typing speed X RSA, *b* = `r apa(summary(model5f)$tTable[10,1], 2)`, *t*(`r summary(model5f)$tTable[10,3]`) = `r apa(summary(model5f)$tTable[10,4],2)`, *p* = `r apa(summary(model5f)$tTable[10,5],3,F)` were the only significant interactions. 

HERE I THINK WE SHOULD BREAK THIS DOWN BY RSA SINCE IT'S THE SIMILAR ONE BETWEEN THE TWO. 

For real words, the three-way interactions of finger switch X hand switch X RSA, *b* = `r apa(summary(model5r)$tTable[16,1], 2)`, *t*(`r summary(model5r)$tTable[16,3]`) = `r apa(summary(model5r)$tTable[16,4],2)`, *p* < .001, and speed X finger switch X hand switch, *b* = `r apa(summary(model5r)$tTable[13,1], 2)`, *t*(`r summary(model5r)$tTable[13,3]`) = `r apa(summary(model5r)$tTable[13,4],2)`, *p* = `r apa(summary(model5r)$tTable[13,5],3,F)`, were the largest significant interaction predictors. 
